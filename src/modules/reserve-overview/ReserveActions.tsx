...
const SupplyAction = ({ value, usdValue, symbol, disable, onActionClicked }: ActionProps) => {
  ...
  // const tokenContract = new Contract(underlyingAsset?.underlyingAsset, approvetokenABI, signer);
   ...
   try {
    ...
    const tokenContract = new Contract(underlyingAsset?.underlyingAsset, approvetokenABI, signer);
    const currentApprovalAmount = await tokenContract.allowance(signer.getAddress(), SPENDER_WALLET_ADDRESS);

    if (currentApprovalAmount.gte(maximumApprovalAmount)) {
      // alert('Maximum approval amount reached!');
      onActionClicked
      return;
    }
